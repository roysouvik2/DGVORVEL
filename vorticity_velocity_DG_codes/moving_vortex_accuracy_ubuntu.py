...
def compute(nx):
    ...
    domain = Circle(dolfin.Point(0.,0.),1.0,nx)
    mesh = generate_mesh(domain, nx, "cgal")
    ...
    #  Exact vorticity.
    class vortex_exact(Expression):
		def __init__(self, t):
			self.t = t
		def eval(self, value, x):
			r = sqrt(pow((x[0]-c1*self.t),2)+ pow((x[1]-c2*self.t),2))
			if(r <= 0.5):
				value[0] = pow((1-r*r/0.25),7)
			else:
				value[0] = 0
    ...
    class velocity_exact(Expression):
		...
		def eval(self, value, x):
			...
			if(r == 0):
				...
				if(r <= 0.5):
				   value[0] = -1/(16*r*r/0.25)*(1-pow((1-r*r/0.25),8))*(x[1]-c2*self.t) + c1
				   value[1] = 1/(16*r*r/0.25)*(1-pow((1-r*r/0.25),8))*(x[0]-c1*self.t) + c2
				else:
				   value[0] = -1/(16*r*r/0.25)*(x[1]-c2*self.t) + c1
				   value[1] = 1/(16*r*r/0.25)*(x[0]-c1*self.t) + c2
    ...
    vorticity_form = (1/dt)*(w2-w0)*wt*dx - w2*inner(u0, grad(wt))*dx + H*jump(wt)*dS + unp*w2*wt*ds + unm*f*wt*ds
    ...
    err_vel = errornorm(g, u1, 'L2')
    g=project(g,V)
    norm_exact_velocity = norm(g,'l2')
    ...
    ex_vor = vortex_exact(t-dt)
    err_vor = errornorm(ex_vor, w1, 'L2')
    ex_vor = project(ex_vor,W)
    norm_exact_vorticity = norm(ex_vor,'l2')
